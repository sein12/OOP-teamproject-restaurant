...
public class Server{
  ...
  void reservationsEntry () {   // 예약 입장 확인 메소드
    // DB에서 해당 타임 값 변수 만들면 가져오기
    System.out.println("이번 "+Table.currentTime+"시 타임에 대해 예약 확인해 드리겠습니다.");   // 재윤이가 Table class에 static으로 currentTime변수 만들어주기. Server에서는 Table.currentTime으로 받아와서 쓸거임.

    if(Hall.reservationList[Table.currentTime-12]) {     // 재윤or정우가 Hall class에 reservationList[] static으로 만들어주기 boll type list입니다.
  ...
  void menuGuidance() {   // 모든 메뉴 안내 메소드; 가격도 포함
    ...
    for(int i = 0; i<course_menu.size(); i++) {   // course_menu의 index값 개수만큼 메뉴와 가격을 나란히 차례로 출력하기를 반복
      System.out.print(MenuCustomerPrice.course_menu[i]+"    ");     // 요리명과 가격을 나란히 출력할 수 있도록 print하고 enter안함.
      System.out.println(MenuCustomerPrice.course_price[i]);  // 다음 요리명과 가격을 출력하기 위해 다음 줄로 개행
    }
    System.out.println("**********음료**********");
    for(int j = 0; j<(drink_menu.size(); j++) {  // drink_menu의 index값 개수만큼 메뉴와 가격을 나란히 차례로 출력하기를 반복
      System.out.print(MenuCustomerPrice.drink_menu[j]+"    ");    // 음료명과 가격을 나란히 출력할 수 있도록 print하고 enter안함.
      System.out.println(MenuCustomerPrice.drink_price[j]); // 다음 음료명과 가격을 출력하기 위해 다음 줄로 개행
    }
  }
  ...
  void orderManager(){
    ...
    System.out.println("아래의 메뉴중 원하시는 메뉴의 수량을 입력해주세요.");
    if 
    Main.table1.orderUpdated("황태 해장국", custormerNumber);
  }

    void getFeedbackAndGetPayied() {       // 피드백 받고 결제 받는 메소드; 내가 보내준 코드 정우가 payment.java에 수정해 넣기
        feedbackGetAndCheck feedback_SingletonInst = feedbackGetAndCheck.getInstance();  // 정우가 feedback에 관련된(받고 확인하는 메소드 있는) class 싱글톤으로 만들기
        feedback_SingletonInst.getFeedback();   // 결제하기 전 피드백 받는 함수 호출
      
        Payment payment_inst = new Payment();  // 외부 클래스의 인스턴스 생성
        Payment.CouponandDiscount couponandDiscount_inst = payment_inst.new CouponandDiscount();  // 비정적 중첩 클래스;Payment 내부의 CouponChecker 이 둘의 인스턴스 생성
        System.out.println("결제 도와드리겠습니다.")
        couponandDiscount_inst.checkCouponAndDiscount();        // 쿠폰 확인 후 할인 적용하는 메소드
        Payment.payWithCardCash payWithCardCash_inst = payment_inst.new payWithCardCash();  // 비정적 중첩 클래스;Payment 내부의 CouponChecker 이 둘의 인스턴스 생성
